    ...
    parseUrl (url, fn) {
        if ( typeof url === 'string' ) {
            ...
            try{
                ...
                prefix    = path.extname(path.basename(uobj.path).replace(/\?+.+/g,'')).slice(1);
                aurl      = new RegExp('(\\.)' + prefix + '(\\?.{0,})').exec(uobj.path) || [];
                param     = (aurl[2] && (~this._params.indexOf(aurl[2].slice(1)))) ? aurl[2].slice(1) : '';
                upath     = aurl[2] ? uobj.path.slice(1).match(/(.+\.\w+)(\?.{0,})/)[1] : uobj.path.slice(1);
                isBuild   = (param || this.isCss(prefix))  ? true : false;
            }catch(e){
            ...
            if ( ~this.combo_format.indexOf(prefix) ) {
                ...
                param  = this.isSass(prefix) ? 's' : this.isLess(prefix) ? 'l' : param;
                prefix = this.isCss(prefix) ? 'css' : prefix;
                return fn && fn(this.addPath(resultArr), prefix, param, isBuild);
            } else {
